...
public class AutorView extends javax.swing.JFrame {
     ...
     */
    private List<AutorBEAN> ListaAutorBEANs;
    ...
    private void btnConfirmarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnConfirmarActionPerformed
        try {
            if (verificaCampos()) {
                ...
                autor.setNome_Autor(txNome.getText());
                autor.setAutor_Ativo(ckbAtivo.isSelected());
                autor.setCPF_Autor(txCpf.getText());
                autor.setData_de_Nas(new Date());
                autor.setEmail_Autor(txEmail.getText());
                autor.setRG_Autor(txRg.getText());
                autor.setEndereço_Autor(txEndereco.getText());
                autor.setTelefone_Autor(txTelefone.getText());
                autor.setData_de_Nas(convert_StringToDate(txNascimento.getText()));
                if (cadastroOuAtualiza == 0) {
                    controlerAutor.addAutor(autor);
                } else {
                    autor.setIdAutor(Integer.parseInt(jTable1.getModel().getValueAt(jTable1.getSelectedRow(), 0).toString()));
                    controlerAutor.updateAutor(autor);
    ...
    private void updateTabela() {
        ...
        if (ckbInativos.isSelected()) {
            if (buscar.isEmpty()) {
                ListaAutorBEANs = new ControlerAutor().listaAutores();
            } else {
                ListaAutorBEANs = new ArrayList<>();
                ListaAutorBEANs.addAll(new ControlerAutor().findByNome(buscar));
            }

        } else {
            if (buscar.isEmpty()) {
                ListaAutorBEANs = new ControlerAutor().listaAutoresAtivos();
            } else {
                ListaAutorBEANs = new ArrayList<>();
                ListaAutorBEANs = new ControlerAutor().findByNomeAndAtivo(buscar);
            }

        }
        DefaultTableModel model = new DefaultTableModel(null, new String[]{"ID", "Nome Autor", "Status", "telefone", "CPF", "RG", "Endereço", "Email", "data_de_nas"});
        try {
            jTable1.setModel(model);
            String[] dados = new String[9];
            for (AutorBEAN a : ListaAutorBEANs) {
                dados[0] = String.valueOf(a.getIdAutor());
                dados[1] = a.getNome_Autor();
                dados[2] = a.getAutor_Ativo().toString();
                dados[3] = a.getTelefone_Autor();
                dados[4] = a.getCPF_Autor();
                dados[5] = a.getRG_Autor();
                dados[6] = a.getEndereço_Autor();
                dados[7] = a.getEmail_Autor();
